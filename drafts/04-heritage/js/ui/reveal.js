...
function splitLines(el) {
  ...
  /* Whitespace in the source is recorded per unit rather than emitted
     as its own node, because the rebuild re-spaces the line. Losing it
     is how an inline <em> ends up welded to the word before it. */
  ...
  const pushWords = (text) => {
    ...
    for (const part of parts) {
      ...
      s.textContent = part;
      frag.appendChild(s);
      units.push({ el: s, space: pendingSpace });
      pendingSpace = false;
    }
  };
  ...
  for (const node of Array.from(el.childNodes)) {
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      units.push({ el: node, space: pendingSpace });
      pendingSpace = false;
    }
  }
  ...
  for (const u of units) {
    ...
    u.el.dataset.space = u.space ? '1' : '0';
    current.push(u.el);
  }
  if (current.length) lines.push(current);
  ...
  for (const line of lines) {
    ...
    line.forEach((node, i) => {
      /* Re-space exactly where the source had whitespace, so an inline
         mark that hugged its word keeps hugging and one that did not
         keeps its gap. */
      if (i > 0 && node.dataset.space === '1') {
        inner.appendChild(document.createTextNode(' '));
      }
      delete node.dataset.space;
...
function initReveals({ reducedMotion = false } = {}) {
  ...
  /* ---------- Media ----------
     A clip reveal paired with an inner scale, so the photograph
  ...
  document.querySelectorAll('[data-media-reveal]').forEach((el) => {
    ...
    ScrollTrigger.create({
      ...
      once: true,
      onEnter: () => {
        gsap.to(el, { clipPath: 'inset(0 0 0% 0)', duration: 1.1, ease: 'expo.out' });
  ...
  document.querySelectorAll('.plate:not(.hero) .plate__img').forEach((img) => {
