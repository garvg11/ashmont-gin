...
/* Ground, glow and light per theme. Dark is the brand's navy night.
   Light is a sky, not a white page: the glass has to sit in daylight
...
export function createScene(container, { tier = 'high' } = {}) {
  ...
  function buildEnvironment() {
    ...
    if (theme === 'dark') {
      base.addColorStop(0, '#16294d');
      base.addColorStop(0.48, '#0a142a');
      base.addColorStop(1, '#04080f');
    } else {
      base.addColorStop(0, '#ffffff');
      base.addColorStop(0.46, '#d7e8f8');
      base.addColorStop(1, '#7ba3cc');
    }
    ctx.fillStyle = base;
  ...
  const rim = new THREE.DirectionalLight(palette.rim, theme === 'dark' ? 3.4 : 2.0);
  rim.position.set(3.4, 1.6, -3.2);
  scene.add(rim);

  const fill = new THREE.HemisphereLight(palette.key, palette.fill, theme === 'dark' ? 0.55 : 1.4);
  scene.add(fill);
  ...
  function applyTheme(next) {
    ...
    key.intensity = theme === 'dark' ? 2.6 : 3.4;
    rim.color.set(palette.rim);
    rim.intensity = theme === 'dark' ? 3.4 : 2.0;
    fill.color.set(palette.key);
    fill.groundColor.set(palette.fill);
    fill.intensity = theme === 'dark' ? 0.55 : 1.4;
    if (motes) {
